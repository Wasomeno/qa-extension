...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      sendResponse(res, 200, true, 'GitLab OAuth URL generated', {
        authUrl,
        sessionId: state,
      });
    } catch (error) {
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    try {
      ...
      const gitlabTokens = await gitlabService.exchangeCodeForTokens(
        code as string
      );
      ...
      if (!user) {
        ...
        await authService.updateOAuthTokens(
          user.id,
          'gitlab',
          gitlabTokens,
      ...
      const sessionId =
        state?.toString() || require('crypto').randomBytes(16).toString('hex');
      ...
      try {
        ...
        logger.warn(
          'Failed to store OAuth session in Redis, using memory fallback'
        );
        // Store in module-level memory as fallback
        (global as any).oauthSessions =
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    try {
      ...
      const currentUser = await gitlabService.getCurrentUser(
        connections.gitlab.access_token
      );
      ...
      for (const project of limitedProjects) {
        try {
          const projectMembers = await gitlabService.getProjectMembers(
            project.id
          );
          ...
          logger.debug(
            `Skipping project ${project.id} due to access error:`,
            error
          );
        }
      }
      ...
      sendResponse(
        ...
        200,
        true,
        'GitLab users retrieved successfully',
        users
      );
    } catch (error) {
      ...
      sendResponse(
        ...
        200,
        true,
        'Could not fetch GitLab users, using fallback',
        []
      );
    }
  })
