...
(function () {
  ...
  document.addEventListener('mousemove', e => {
    ...
    const newX = Math.max(
      30,
      Math.min(e.clientX - dragOffset.x, window.innerWidth - 30)
    );
    const newY = Math.max(
      30,
      Math.min(e.clientY - dragOffset.y, window.innerHeight - 30)
    );
  ...
  window.addEventListener('resize', () => {
    const newX = Math.max(
      30,
      Math.min(currentPosition.x, window.innerWidth - 30)
    );
    const newY = Math.max(
      30,
      Math.min(currentPosition.y, window.innerHeight - 30)
    );
